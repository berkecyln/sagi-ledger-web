...
import { aggregateByDescriptionAndAccount, aggregateByDescriptionWithAccounts, formatEuro } from '../utils/aggregations';
...
const th = 'px-4 py-2 text-left text-xs font-medium text-ink-faint uppercase tracking-wide';
const thR = 'px-4 py-2 text-right text-xs font-medium text-ink-faint uppercase tracking-wide';
  ...
  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Income Column */}
        <div className="border border-stroke rounded-lg overflow-hidden flex flex-col">
          ...
          <div className="bg-card flex-1">
            {aggIncome.length === 0 ? (
              <p className="text-sm text-ink-faint px-4 py-8 text-center">No income this month</p>
            ) : (
              <table className="w-full text-sm">
                ...
                <tbody>
                  {aggIncome.map((row) => (
                    <tr key={`${row.description}-${row.account}`} className="border-b border-stroke-light last:border-0">
                      <td className="px-4 py-3 text-ink">{row.description}</td>
                      <td className="px-4 py-3"><AccountTag account={row.account} color={accountColors[row.account]} /></td>
                      <td className="px-4 py-3 text-right font-medium text-income">{formatEuro(row.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-income-surface/50 border-t border-income-stroke px-4 py-2.5 flex justify-between items-center">
            <span className="text-xs font-semibold text-income uppercase tracking-wide">Total</span>
            <span className="font-semibold text-income">{formatEuro(totalIncome)}</span>
          </div>
        ...
        <div className="border border-stroke rounded-lg overflow-hidden flex flex-col">
          ...
          <div className="bg-card flex-1">
            {aggExpense.length === 0 ? (
              <p className="text-sm text-ink-faint px-4 py-8 text-center">No expenses this month</p>
            ) : (
              <table className="w-full text-sm">
                <thead className="border-b border-stroke-light">
                  <tr>
                ...
                <tbody>
                  {aggExpense.map((row) => (
                    <tr key={row.description} className="border-b border-stroke-light last:border-0">
                      <td className="px-4 py-3 text-ink">{row.description}</td>
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap gap-1">
                          {row.accounts.map((acc) => (
                            <AccountTag key={acc} account={acc} color={accountColors[acc]} />
                          ))}
                        </div>
                      </td>
                      <td className="px-4 py-3 text-right font-medium text-expense">{formatEuro(row.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-expense-surface/50 border-t border-expense-stroke px-4 py-2.5 flex justify-between items-center">
            <span className="text-xs font-semibold text-expense uppercase tracking-wide">Total</span>
            <span className="font-semibold text-expense">{formatEuro(totalExpense)}</span>
          </div>
