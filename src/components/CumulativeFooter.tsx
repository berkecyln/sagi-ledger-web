...
export default function CumulativeFooter() {
  ...
  return (
    <div className="border-t border-border bg-card p-4 mt-auto flex justify-center items-center relative">
      <div className="max-w-7xl flex flex-wrap gap-4 items-center justify-center text-sm">
        <span className="mr-2 uppercase tracking-wider text-xs font-semibold text-muted-foreground">
          Total Balances
        </span>
        {activeBalances.map(([account, amount]) => (
          <div
            key={account}
            className="flex items-center gap-2 bg-background px-3 py-1.5 rounded-full shadow-sm border border-border"
          >
            <div
            ...
            <span className={"font-semibold " + (amount < 0 ? "text-red-500" : "text-green-500")}>{formatEuro(amount)}</span>
          </div>
        ))}
      </div>
